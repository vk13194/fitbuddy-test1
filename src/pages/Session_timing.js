...
export default function Session_timing() {
    ...
    const sessionDropDownList = [ <
        ...
        id = "firstRowColumn1" >
        <
        select className = { st.exercise_button }
        onChange = {
            (event) => SessionChangeHandler(event)
        } >
        <
        option selected > & #60;select&# 62; < /option> <
        option > Yoga < /option> <
        option > Gymnastics < /option> < /
        select > {
            addButtonStatus && < button className = { st.plus_button }
            id = "addButton"
            onClick = {
                (event) => addNewRow()
            } > < img src = './assets/plus_icon.png' > < /img></button >
        } <
        /td>
    ];
    const timeDropDownList = [ < td id = "firstRowColumn2" >
        ...
        select className = { st.exercise_button }
        onChange = {
            (event) => TimeChangeHandler(event)
        } >
        <
        option selected > & #60;select&# 62; < /option> <
        option > 10 Am to 11 Am < /option> <
        option > 11 Am to 11: 40 Am < /option> < /
        select > <
        /td>
    ];
...
<ul id="days_ul_list" className={st.items} >
 ...
 <li><input type="checkbox" />Tuesday</li>
    ...
    return ( <
        ...
        <
        table id = "mainTable"
        className = { st.session_timing_table } >
        <
        tr >
        <
        ...
        th colSpan = "2" > Day < /th> < /
        tr > <
        tr id = "firstRow" > { sessionDropDownList } {
            timeStatus && ( < td colSpan = "2"
                id = "firstRowColumn2" >
                <
                span className = { st.exercise_button_time }
                onChange = {
                    (event) => TimeChangeHandler(event)
                }
                style = {
                    { width: '300px', height: '40px', display: 'flex', justifyContent: "center", alignItems: 'center' }
                } >
                From < input style = {
                    { "border-radius": "5px" }
                }
                type = "time" > < /input> To <input style={{ "border-radius": "5px" }} type="time"></input >
                <
                /span> < /
                td > )
        } {
            dayStatus && ( < td colSpan = "2"
                id = "firstRowColumn3"
                onChange = {
                    (event) => DayChangeHandler(event)
                } >
                <
                span className = { st.exercise_button_span }
                onClick = {
                    () => dayListHandler()
                } > Select Day & #x25BC; <
